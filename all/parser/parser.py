...
from all.common import expressions, statements
...
class Parser:
    ...
    def __parse_expr(self, src: list[dict]):
        position = 0
        # utils.py veya expressions.py içinde
        BINARY_LEFT_ALLOWED = {
            token_types.TT_INTEGER_TOKEN,
            token_types.TT_FLOAT_TOKEN,
            token_types.TT_HEX_TOKEN,
            token_types.TT_BINARY_TOKEN,
            token_types.TT_IDENTIFIER_TOKEN,
            statements.STMT_ROUND_PAREN,
            expressions.EXPR_NUMBER,
            expressions.EXPR_VARIABLE,
            expressions.EXPR_CALL,
            expressions.EXPR_ARRAY,
            expressions.EXPR_STRUCT,
            expressions.EXPR_MEMBER,
            expressions.EXPR_LOGICAL_NOT,
            expressions.EXPR_BITWISE_NOT,
            expressions.EXPR_NEGATE,
            expressions.EXPR_ADD,
            expressions.EXPR_SUB,
            expressions.EXPR_MUL,
            expressions.EXPR_DIV,
            expressions.EXPR_INT_DIV,
            expressions.EXPR_MOD,
            expressions.EXPR_POW,
            expressions.EXPR_BIT_AND,
            expressions.EXPR_BIT_OR,
            expressions.EXPR_BIT_XOR,
            expressions.EXPR_SHIFT_LEFT,
            expressions.EXPR_SHIFT_RIGHT,
            expressions.EXPR_LOGICAL_AND,
            expressions.EXPR_LOGICAL_OR,
            expressions.EXPR_EQUAL,
            expressions.EXPR_NOT_EQUAL,
            expressions.EXPR_LESS,
            expressions.EXPR_GREATER,
            expressions.EXPR_LESS_EQ,
            expressions.EXPR_GREATER_EQ,
        }

        while position < len(src):
            token = src[position]
            
            if token["type"] == token_types.TT_NOT_TOKEN:
                src.pop(position)
                if (not position < len(src) and 
                    src[position]["type"]
                    not in BINARY_LEFT_ALLOWED
                ):
                    raise errors.Unparseable_NOT_error(
                        token["line"],
                        token["column"],
                        token["file"],
                    )
                value = src.pop(position)
                if value["type"] == statements.STMT_ROUND_PAREN:
                    value["body"] = self.__parse_expr(value["body"])
                src.insert(
                    position,
                    {
                        "type": expressions.EXPR_LOGICAL_NOT,
                        "line": value["line"],
                        "column": value["column"],
                        "file": value["file"],
                        "value": value,
            ...
            if (
                token["type"] == token_types.TT_SUB_TOKEN
                and position > 0
                and src[position - 1]["type"] not in BINARY_LEFT_ALLOWED
            ):
                src.pop(position)
                if (position < len(src) and src[position]["type"] not in BINARY_LEFT_ALLOWED):
                    raise errors.Unparseable_NEGATE_error(
                        token["line"],
                        token["column"],
                        token["file"],
                    )
